...
class tx_dkdfeuserbelogin_pi1 extends tslib_pibase {
	...
	function main($content,$conf)	{
		...
		if ($_COOKIE["be_typo_user"]) {
			# patch by Mauro Lorenzutti of webformat.com to work with DBAL
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*", "be_sessions", "ses_id='".$_COOKIE["be_typo_user"]."'");
			if ($GLOBALS['TYPO3_DB']->sql_num_rows($res)==1)	{
		...
		if(!$is_be_user&&$is_fe_user) {
			# patch by Mauro Lorenzutti of webformat.com to work with DBAL
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery("be.username, be.password, fe.tx_dkdfeuserbelogin_relatedbeuser", "fe_users AS fe, be_users AS be", "fe.uid=".$fe_user['ses_userid']." AND be.uid=tx_dkdfeuserbelogin_relatedbeuser AND be.deleted!=1");

			if ($GLOBALS['TYPO3_DB']->sql_num_rows($res)==1)	{
				// login für backend user
				...
				$content.='
					...
					<input type="Hidden" name="redirect_url" value="../index.php?id='.$page_id.'">
					...
					<input type="Submit" name="commandLI" value="' . $this->pi_getLL('login', 'CMS LogIn') . '">
					</form>';
			}
		}

		if($is_be_user) {
				$content.= str_replace('###LOGOUTLINK###','typo3/logout.php?redirect=../index.php?id='.$page_id.'&ATBE=1&sendLogoutSignal=1',$this->conf["logoutButton"]);
			}
